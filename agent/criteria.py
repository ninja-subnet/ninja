"""Acceptance-checklist extraction from the task text.

Ported from the 081 king lineage: bullet/numbered requirements plus
integration/UI/refactor hints. Injected into the task prompt and once mid-run
so multi-requirement tasks stop shipping half-covered patches (duel-101923
r11/r30/r38-class losses).
"""
...
def extract_criteria(issue: str) -> list[str]:
    ...
    if len(out) < 2:
        for hint in (
            "Implement every concrete behavior the task names in reachable code.",
            "Wire new symbols into an existing call path (import, route, parent, config).",
        ):
            if hint not in out:
                out.append(hint)
    for hint in _integration_hints(issue or ""):
        if hint not in out:
            out.append(hint)
    return out[:15]
...
def format_checklist(criteria: list[str]) -> str:
    ...
    return (
        ...
        "Verify every item against your diff before `echo` submit:\n"
        f"{rows}\n"
    )
