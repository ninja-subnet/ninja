...
from agent.agent_loop import (
    ...
    DEFAULT_MAX_TOKENS as _LOOP_MAX_TOKENS,
    AgentRunConfig,
    run_agent_loop,
)
...
# Backstop only, deliberately far above what 600s of wall clock can fit:
# wrap-up pacing comes from the wall-clock notes (agent_loop._time_pressure_note),
# so the clock — not this cap — is what ends a healthy run.
DEFAULT_MAX_STEPS = int(os.environ.get("AGENT_MAX_STEPS", "5000"))
...
DEFAULT_MAX_TOKENS = int(os.environ.get("AGENT_MAX_TOKENS", str(_LOOP_MAX_TOKENS)))
...
# The solver endpoint's window (vLLM --max-model-len; GET /v1/models reports 32768).
# The loop compacts to max_model_len - max_tokens so the prompt and the reply's
# reservation always fit together; overshooting it is a 400, not a truncation.
MAX_MODEL_LEN = int(os.environ.get("AGENT_MAX_MODEL_LEN", str(DEFAULT_MAX_MODEL_LEN)))
